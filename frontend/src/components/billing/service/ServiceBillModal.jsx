            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-4 p-4 bg-green-50 rounded-lg border border-green-200">
                        <h3 className="text-lg font-medium text-green-800">Customer Details</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-green-700">Client Name *</label>
                                <Input
                                    value={formData.customer.name}
                                    onChange={(e) => handleCustomerChange('name', e.target.value)}
                                    placeholder="Enter client name"
                                    className={`border-green-300 focus:border-green-500 focus:ring-green-500 ${errors['customer.name'] ? 'border-red-300' : ''}`}
                                />
                                {errors['customer.name'] && (
                                    <p className="text-red-500 text-sm">{errors['customer.name']}</p>
                                )}
                            </div>
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    className={`border-green-300 focus:border-green-500 focus:ring-green-500 ${errors['customer.phone'] ? 'border-red-300' : ''}`}
                                />
                                {errors['customer.phone'] && (
                                    <p className="text-red-500 text-sm">{errors['customer.phone']}</p>
                                )}
                            </div>
                        </div>
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-green-700">Address (Optional)</label>
                            <Textarea
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <h3 className="text-lg font-medium text-green-800">Service Details</h3>
                            <Button
                                type="button"
                                variant="outline"
                                size="sm"
                        ...
                        {formData.serviceItems.map((item, index) => (
                            <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 border border-green-200 rounded-lg bg-green-25">
                                <div className="space-y-2">
                                ...
                                <div className="space-y-2 md:col-span-2">
                                    <label className="text-sm font-medium text-green-700">Service Description *</label>
                                    <Input
                                        value={item.description}
                                        onChange={(e) => handleServiceItemChange(index, 'description', e.target.value)}
                                        placeholder="Describe the service provided"
                                        className={`border-green-300 focus:border-green-500 focus:ring-green-500 ${errors[`serviceItems.${index}.description`] ? 'border-red-300' : ''}`}
                                    />
                                    {errors[`serviceItems.${index}.description`] && (
                                        <p className="text-red-500 text-sm">{errors[`serviceItems.${index}.description`]}</p>
                                    )}
                                </div>
                                <div className="space-y-2">
                                    ...
                                    <div className="flex">
                                        <Input
                                            ...
                                            placeholder="0.00"
                                            className={`border-green-300 focus:border-green-500 focus:ring-green-500 ${errors[`serviceItems.${index}.amount`] ? 'border-red-300' : ''}`}
                                        />
                                    ...
                                    {errors[`serviceItems.${index}.amount`] && (
                                        <p className="text-red-500 text-sm">{errors[`serviceItems.${index}.amount`]}</p>
                                    )}
                                </div>
                            </div>
                        ))}
                    ...
                    {/* Total and Actions */}
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-4 border-t border-green-200">
